  ...
  for (const currencyCodea in data) {
    if (Object.hasOwnProperty.call(data, currencyCodea)) {
      ...
      if (currencya.code == currencyRuleCode) {
        ...
        globalSellingRuleValue = sellingRuleValue;
        if (globalSellingRuleValue != undefined) {
          updateTableAndIcon(currencytitle, sellingRuleValue, ruleOrder,rulesellmore);
        }
      }
    }
    break;
...
function updateTableAndIcon(currencytitle, sellingRuleValue, ruleOrder,rulesellmore) {
  ...
  if (element.length > 0) {
    ...
    var formattedNumber = formatNumber(new Intl.NumberFormat("de-DE").format(sellingRuleValue));
...
function alisUpdateTableAndIcon(currencytitle, sellingRuleValue, rulebuymore, ruleOrder) {
  var element = $('tr[data-order="' + ruleOrder + '"]');
  if (element.length > 0) {
...
function formatNumber(sellingRuleValue) {
  ...
  if (decimalIndex > 0) {
      ...
      if (secondDecimalIndex > 0) {
          numberString = numberString.slice(0, secondDecimalIndex) + ',' + numberString.slice(secondDecimalIndex + 1);
      }
  }
  return numberString;
}
